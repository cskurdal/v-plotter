...
def main():
	...
	if args.setup:
		...
		while abs(x1 - p[0]) > maxStepSize or abs(y1 - p[1]) > maxStepSize:
			...
			dirX = -1.0 if x2 < x1 else 1.0
			dirY = -1.0 if y2 < y1 else 1.0
			xStep = maxStepSize * dirX
		        print " X step ", xStep
			yStep = xStep * (y2 - y1) / (x2 - x1)
			x1, y1 = moveTo(x1, y1, x1 + xStep, y1 + yStep, c)			    
		    elif abs(y1 - float(p[1])) > maxStepSize:
		        dirX = -1.0 if float(p[0]) < x1 else 1.0
			dirY = -1.0 if float(p[1]) < y1 else 1.0
			yStep = maxStepSize * dirY
		        print " Y step ", yStep
			xStep = yStep * (x2 - x1) / (y2 - y1)
         		x1, y1 = moveTo(x1, y1, x1 + xStep, y1 + yStep, c)

		#Do this at the end of the looping above so that we ensure we went to the point
		x1, y1 = moveTo(x1, y1, float(p[0]), float(p[1]), c)
